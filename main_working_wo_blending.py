...
def Perspective_warping(img1, img2, img3):
	...
	img1 = cv2.warpPerspective(img2, M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), borderMode=cv2.BORDER_TRANSPARENT)

	(M, pts1, pts2, mask) = getTransform(img3, img1, 'homography')
	output_image = cv2.warpPerspective(img3, M, (img1.shape[1],img1.shape[0]), dst=img1.copy(), borderMode=cv2.BORDER_TRANSPARENT)

	# debug block to compare image output result
	# sample_output = cv2.imread("example_output1.png", 0)
	# print "RMSD= ", RMSD(output_image, sample_output)
...
def stitchCylindrical(baseImage, warpedImage, warpedImageMask):
	height, width = warpedImageMask.shape
	for x in range(0, height-1):
		for y in range(0, width-1):
			if(warpedImageMask[x][y] == 255):
				baseImage[x][y] = warpedImage[x][y]
	return baseImage
...
def Cylindrical_warping(img1, img2, img3):
	...
	warped_mask_img2 = cv2.warpAffine(mask_img2, M, (img1.shape[1],img1.shape[0]))
	img1 = stitchCylindrical(img1, warped_img2, warped_mask_img2)
	...
	warped_mask_img3 = cv2.warpAffine(mask_img3, M, (img1.shape[1],img1.shape[0]))
	output_image = stitchCylindrical(img1, warped_img3, warped_mask_img3)
